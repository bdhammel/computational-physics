...
/* Position of moon */
const double xm = 60.27;
const double GMm = 1/81.17;
...
double get_acceleration(double x, double y, double *ax, double *ay) {
    ...
    double re = sqrt(x*x + y*y);    // distance from earth
    double rm = sqrt( pow( (xm - x), 2) +  pow( y, 2));    // distance from moon
    
    double aex = -(GM / (pow(re, 3)) ) * x;
    double aey = -(GM / (pow(re, 3)) ) * y;
    double amx = -(GMm / (pow(rm, 3)) ) * ( x - xm );
    double amy = -(GMm / (pow(rm, 3)) ) * y;
...
int main(void) {
    ...
    x[0]=-1.; y[0]=0.0; vx[0]= 0.0; vy[0]=1.40081635; 
